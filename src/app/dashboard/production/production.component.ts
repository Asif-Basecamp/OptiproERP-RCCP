...
export class ProductionComponent implements OnInit {
  ...
  constructor(private router: Router, private toastrService: ToastrService, private dialogService: DialogService, private datePipe: DatePipe, private dash: DashboardService,private prod: ProductionService) {}
    ...
    data => {
      if(data != undefined && data != null){
        if(data.length > 0){
         ...
         this.toastrService.error(this.language.no_record_found);    
        }       
      }
      else {
       this.toastrService.error(this.language.no_record_found);    
 ...
 gridRowSelectDocEntry(evt){
  ...
  GetExplosionData() {
   if(this.ItemCodeFrom && !this.ItemCodeTo){
     this.toastrService.error(this.language.item_code_to_msg);
   }else if(!this.ItemCodeFrom && this.ItemCodeTo){
     this.toastrService.error(this.language.item_code_from_msg);
   }else{
   let gridItemSelect = [];
   this.GridViewSelected = (e: RowArgs) => gridItemSelect.indexOf(e.dataItem.Code) >=0 ;
